...
app.post("/storeToken", (req, res) => {
  uploadImages(req, res, function (err) {
    ...
    const query =
      "INSERT INTO `tbl_nfts` (`nft_image`, `nft_name`, `nft_url`, `nft_description`, `nft_mint_date`, `nft_pre_mint_spots`, `nft_pre_mint_price`, `nft_extra_images`, `nft_link_domain`, `nft_link_discord`, `nft_link_twitter`, `nft_link_instagram`, `nft_link_reddit`,`nft_wallet_address`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
app.post("/updateToken", (req, res) => {
  uploadImages(req, res, function (err) {
    ...
    const query =
      "UPDATE tbl_nfts SET `nft_image`=? , `nft_name`=?,`nft_url`=?,`nft_description`=?,`nft_mint_date`=?,`nft_pre_mint_spots`=?,`nft_pre_mint_price`=?,`nft_extra_images`=?,`nft_link_domain`=?,`nft_link_discord`=?,`nft_link_twitter`=?,`nft_link_instagram`=?,`nft_link_reddit`=? WHERE nft_id =?";
...
app.get("/getNftById/:id", (req, res) => {
  db.query(
    "SELECT * FROM tbl_nfts WHERE nft_id=" + req.params.id,
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/storeInvestment", (req, res) => {
  const query =
    "INSERT INTO `tbl_investments` (`investment_amount`, `nft_id`, `user_wallet_address`, `investment_status`) VALUES (?, ?, ?, ?)";
...
app.post("/getInvestment/:nft_id", (req, res) => {
  ...
  db.query(
    "SELECT * FROM tbl_investments WHERE nft_id=" +
      req.params.nft_id +
      " AND user_wallet_address=" +
      req.body.user_wallet +
      "ORDER BY investment_timestamp",
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/refundInvestment", (req, res) => {
  const query =
    "UPDATE tbl_investments SET `investment_status`=? WHERE nft_id =? AND user_wallet_address=?";

  db.query(
    query,
    [req.body.investment_status, req.body.nft_id, req.body.user_wallet_address],
    (err, data) => {
      if (err) {
        return res.json(err);
      }
...
app.post("/storeUser", (req, res) => {
  const query =
    "INSERT INTO `users` (`user_wallet_address`, `user_uid`) VALUES (?, ?)";

  db.query(
    query,
    [req.body.user_wallet_address, req.body.user_uid],
    (err, data) => {
      if (err) {
        return res.json(err);
      }
...
app.get("/getUserByWalletAddress/:id", (req, res) => {
  db.query(
    "SELECT * FROM users WHERE user_wallet_address=" + req.params.id,
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/updateUser", upload.single("user_image"), (req, res) => {
  bcrypt
    ...
    .then((hash) => {
      ...
      console.log(userId);
      const query =
        "UPDATE users SET `user_name`=? , `user_email`=?,`user_password`=?,`user_image`=?,`user_interests`=? WHERE user_id =?";
      const values = [
        req.body.user_name,
        req.body.user_email,
        hash,
...
app.post(
  ...
  upload.single("home_background_image"),
  (req, res) => {
    const query = "UPDATE home SET `home_background_image`=? WHERE home_id =?";
    db.query(query, [req.file.filename, req.body.home_id], (err, data) => {
      if (err) {
        return res.json(err);
      }
      return res.json(data);
...
app.post("/updateHomeTwitterLink", (req, res) => {
  ...
  db.query(
    query,
    [req.body.home_twitter_link, req.body.home_id],
    (err, data) => {
      if (err) {
        return res.json(err);
      }
...
app.post("/updateHomeButtonText", (req, res) => {
  ...
  db.query(
    query,
    [req.body.home_signup_button_text, req.body.home_id],
    (err, data) => {
      if (err) {
        return res.json(err);
      }
...
app.post("/updateWelcomeMessage", (req, res) => {
  const query =
    "UPDATE welcome_message SET `welcome_message_text`=? WHERE welcome_message_id =?";
  db.query(
    query,
    [req.body.welcome_message_text, req.body.welcome_message_id],
    (err, data) => {
      if (err) {
        return res.json(err);
      }
...
app.post("/updateEmailCredentials", (req, res) => {
  const query =
    "UPDATE email_credentials SET `email`=?, `password`=? WHERE email_credentials_id =?";
  db.query(
    query,
    [req.body.email, req.body.password, req.body.email_credentials_id],
    (err, data) => {
      if (err) {
        return res.json(err);
      }
